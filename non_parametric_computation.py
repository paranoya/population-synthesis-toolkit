...
u, u_abs, g, g_abs, r, r_abs, SDSS_V_max = np.loadtxt(
        'SDSS/photometry.dat', usecols=(0, 1, 2, 3, 4, 5, 6), unpack=True)
models = Model_grid()
GOOD_MET = [5, 10, 15, 20]
...
plt.contourf(g_r_edges[0:-1], u_r_edges[0:-1], hist_colors.T, cmap='Greys_r')
...
plt.contourf(r_edges[0:-1], u_r_edges[0:-1], hist_colmag.T, cmap='Greys_r')
try:
...
plt.contourf(r_edges[0:-1], g_r_edges[0:-1], hist_colmag.T, cmap='Greys_r')
try:
...
for i, Z in enumerate(GOOD_MET):
    ...
    plt.contourf(logmass_edges[0:-1], tau_edges[0:-1], hist_tau.T, cmap='Greys_r')
    try:
