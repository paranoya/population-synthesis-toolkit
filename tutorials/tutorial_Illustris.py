...
for i, row in enumerate(csv_reader):
    ...
    dust_polynomial.append(np.array([float(k) for k in row[-4].split()]))
    dust_polynomial_error.append(np.array([float(k) for k in row[-3].split()]))
    poly_dust = np.array([float(k) for k in row[-4].split()])
    ...
    poly_mass_fraction = np.array([float(k) for k in row[0].split()])
    

    lbt_index = abs(test_lookback_time.to_value()-lbt).argmin()
    if len(poly_mass_fraction)>1000:
        p = poly_mass_fraction[lbt_index]
        # poly_mass_fraction_error = np.array([float(k) for k in row[2].split()])
