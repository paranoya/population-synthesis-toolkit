...
# -*- coding: utf-8 -*-
"""
Created on Sat Oct 30 13:51:47 2021
...
import sys
sys.path.append('/home/pablo/population-synthesis-toolkit/pst/')
import SSP
...
model_wl = ssp_model.spectrum[0, 0].spectral_axis
...
for file in files:
    ...
    sed, weights = ssp_model.compute_SED(time=lookback_time*u.Gyr,
                                         mass=cum_mass*u.Msun,
                                         metallicity=mean_met)
    illustris_mags = []
    L_lambda = sed.to(u.erg/(u.s)).value/model_wl.value
    dist = 4 * np.pi * (10*u.pc)**2
    for _filter in filters:
        luminosity = _filter.get_flux(model_wl.value, L_lambda).value
        # Absolute magnitude
...
np.savetxt('data/IllustrisTNG100_galaxy_bin_classification_all',
           data.T,
...
np.savetxt('data/IllustrisTNG100_galaxy_bin_classification_subsample',
