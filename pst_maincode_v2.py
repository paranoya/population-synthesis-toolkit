...
n_runs = 1 #Number of runs --> Helps to divide the work into different simultaneus chunks
current_run = 0 #Current run (0 --> n_runs-1)


#NEED read the lines [subhalo_name [Fnu] [Fnu_error]]
real_models = {}
input_file = os.path.join(os.getcwd(),'input', '{}_input'.format(target_mfh))
for line in input_file.readlines():
    subhalo = line[0]
    real_models[subhalo] = {}
    real_models[subhalo]['Fnu_obs'] = line[1]
    real_models[subhalo]['error_Fnu_obs'] = line[2]
...
#    fnu_error = []
    ...
    for i, filter_name in enumerate(obs_filters):
        photo = observables.Filter( wavelength = ssp.wavelength, filter_name = filter_name)
...
##################################################################################
#Main code
output_path = os.path.join(os.getcwd(), 'output', 'results_{}'.format(target_mfh)) #Change folder name as desired
...
for number_model, subhalo in enumerate(real_models):
    ...
    if target_mfh == 'Illustris':
        model_test = models.Tabular_MFH(target['time'], 
                                            target['mass_formed'], 
    ...
    Fnu_obs = target['Fnu_obs']
    error_Fnu_obs = target['error_Fnu_obs']        
        ...
        for av_index, A_V_model in enumerate(model_A_V_grid):
            ...
            for N in N_range:
                for t_initial, t_final in t_grid:
                    basis = models.Polynomial_MFH_fit(N, ssp, obs_filters, obs_filters_wl, t, t0, Z_i,                                                  
                    ...
                    poly_fit = models.Polynomial_MFH(Z=Z_i,
                                               coeffs=coeffs,
                    ...
                    for t_start, t_end in zip(t_zeros[:-1], t_zeros[1:]): #For each combination of start-end of the zeros
                        ...
                        polynomial_model = models.Polynomial_MFH(Z=Z_i, 
